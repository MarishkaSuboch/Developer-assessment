...
namespace DataExporter
{
    public class ExporterDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Note>().HasData(new Note() { Id = 1, PolicyId = 1, Text = "Some notes for 1 policy" },
                new Note() { Id = 2, PolicyId = 2, Text = "Some notes for 2 policy" },
                new Note() { Id = 3, PolicyId = 3, Text = "Some notes for 3 policy" },
                new Note() { Id = 4, PolicyId = 1, Text = "Some other notes for 1 policy" },
                new Note() { Id = 5, PolicyId = 1, Text = "One more notes for 1 policy" }
            );

            modelBuilder.Entity<Policy>().HasMany(p => p.Notes).WithOne().HasForeignKey(p => p.PolicyId).IsRequired();
